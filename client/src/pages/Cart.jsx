...
const Cart = () => {
    const { products, currency, cartItems, removeFromCart, getCartCount, updateToCartItem, navigate, getCartAmount, axios, user, setCartItems } = useAppContext();
  ...
  const placeOrder = async () => {
    try {
      ...
      if (paymentOption === "COD") {
        ...
        // Online Payment
        const amount = Math.round(getCartAmount() * 1.02); // 2% tax included
        const { data } = await axios.post("/api/payment/create-order", { amount,addresses, });
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                            ...
                            <div>
                                ...
                                <div className="font-normal text-gray-500/70">
                                    ...
                                    <div className='flex items-center'>
                                        <p>Qty:</p>
                                        <select onChange={e=>updateToCartItem(product._id, Number(e.target.value))} value = {cartItems[product._id]} className='outline-none'>
                ...
                <button className="group cursor-pointer flex items-center mt-8 gap-2 text-indigo-500 font-medium">
                    <img onClick ={()=>{navigate("/products"); scrollTo(0,0)}} src={assets.arrow_right_icon_colored} alt="arrow" className="group-hover:translate-x-1 transition" />
                    Continue Shopping
                </button>
